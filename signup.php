...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<script>alert('Wow! User Registration Completed. Continue with your credentials in Signin page') </script>";
				$username = "";
				$email = "";
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
...
<head>
    <meta charset="utf-8">
    <title>Sign up - Open PRO Demo</title>
    ...
    <div class="ip oo ss o_">
        ...
        <header class="tj su ns">
            <div class="sm nd au hk">
                <div class="ip ol od iy">
                    <div class="sq rl"> <a class="ic" href="index.html" aria-label="Cruip"> <svg
                                class="sl ij ae text-purple-600" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                                <path
                    ...
                    <nav class="im pv de">
                        <ul class="ip sz oh ou ol">
                            ...
                            <li class="tq" x-data="{ open: false }" @mouseenter="open = true"
                                @mouseleave="open = false"> <a class="ls cx au al ip ol lz cr cf" href="#0"
                                    aria-haspopup="true" :aria-expanded="open" @focus="open = true"
                                    @focusout="open = false" @click.prevent=""> Support <svg
                                        class="sa ib ae lf ot r_ sq" viewBox="0 0 12 12"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M10.28 4.305L5.989 8.598 1.695 4.305A1 1 0 00.28 5.72l5 5a1 1 0 001.414 0l5-5a1 1 0 10-1.414-1.414z">
                                        </path>
                                    </svg> </a>
                                <ul class="oe tj nn nt sh ud al ry oq" x-show="open" x-transition:enter="lz cl cs p"
                                    x-transition:enter-start="l_ n" x-transition:enter-end="lq v"
                                    x-transition:leave="lz cl cs" x-transition:leave-start="lq"
                                    x-transition:leave-end="l_" x-cloak="">
                                    <li> <a class="fk fy li ck ip al au le" href="contact.html" @focus="open = true"
                                            @focusout="open = false">Contact us</a> </li>
                                    <li> <a class="fk fy li ck ip al au le" href="help.html" @focus="open = true"
                                            @focusout="open = false">Help center</a> </li>
                                    <li> <a class="fk fy li ck ip al au le" href="404.html" @focus="open = true"
                                            @focusout="open = false">404</a> </li>
                                </ul>
                            </li>
                        </ul>
                        <ul class="ip sz oh ou ol">
                            <li> <a class="fk text-purple-600 cx au av ip ol lz cr cf" href="signin.php">Sign in</a>
                            </li>
                            <li> <a class="tn la uh ch rp" href="signup.php">Sign up</a> </li>
                        </ul>
                    </nav>
                    <div class="pg" x-data="{ expanded: false }"> <button class="tm" :class="{ 'active': expanded }"
                            @click.stop="expanded = !expanded" aria-controls="mobile-nav" :aria-expanded="expanded">
                            <span class="tx">Menu</span> <svg class="sp ix ae ls cx lz cr cf" viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg">
                                <rect y="4" width="24" height="2" rx="1"></rect>
                                <rect y="11" width="24" height="2" rx="1"></rect>
                                <rect y="18" width="24" height="2" rx="1"></rect>
                            </svg> </button>
                        <nav id="mobile-nav" class="tj nn no nr su au hk o_ ct co cf" x-ref="mobileNav"
                            ...
                            <ul class="ud au al">
                                ...
                                <li> <a class="fk su id ol op x uo au al nq oq la uh ch lz cr cf" href="signup.php">Sign
                                        up</a> </li>
                            </ul>
                        </nav>
                    </div>
        ...
        <main class="sz">
            ...
            <div class="tq sm nd ig tk" aria-hidden="true">
                <svg class="tj ne nt p d ie" width="722" height="320" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                                ...
                                <div class="ip ou nv">
                                    <div class="su aa">
                                        <button class="tt ac la up cp su tq ip ol">
                                            <svg class="sf iw ae la lj sq ng" viewBox="0 0 16 16"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path
                                            ...
                                            <span class="sj aq az rc">Sign up with Google</span>
                                        </button>
                                    </div>
                                </div>
                            </form>
                            <div class="ip ol ny">
                                <div class="te ui ue sz rh" aria-hidden="true"></div>
                                <div class="li">Or, register with your email</div>
                                <div class="te ui ue sz rp" aria-hidden="true"></div>
                            </div>
                            <form action="" method="POST" class="login-email">
                                
                                <div class=" ts su ls">
                                    <input type="text" placeholder="Username" class="ts su ls" name="username"
                                ...
                                <div class=" ts su ls">
                                    <input type="email" class="ts su ls"  placeholder="Email" name="email"
                                        value="<?php echo $email; ?>" required=>
                                </div>
                                <div class=" ts su ls">
                                    <input type="password" class="ts su ls"  placeholder="Password" name="password"
                                        value="<?php echo $_POST['password']; ?>" required>
                                </div>
                                <div class=" ts su ls">
                                    <input type="password" class="ts su ls"  placeholder="Confirm Password" name="cpassword"
                                        value="<?php echo $_POST['cpassword']; ?>" required>
                                </div>
                                <div class="fy lf fp">
                                I agree to continue as per the VPRojects <a
                                        class="lw li cx cj lz cr cf" href="#0">Privacy Policy</a>.
                                </div>
                                <div class="ip ou nv ro">
                                <button name="submit" class="tt la uh ch su">Register</button>
                                    </div>
                                </div>
                            </form>
                            <div class="li fp ro">
